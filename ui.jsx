...
function LevelBadge({ level, small }) {
  const map = {
    Principiante: { bg: '#FEF3E7', fg: '#B86A1A' },
    Intermedio:   { bg: CT.greenSoft, fg: CT.greenDeep },
    Avanzado:     { bg: '#E9EDFB', fg: '#3850B8' },
  };
  const c = map[level] || map.Intermedio;
  return (
    <span style={{
...
function Notice({ children, tone = 'success', style = {} }) {
  const palette = tone === 'error'
    ? { bg: '#FDF0ED', fg: '#A04020', border: '#E6B2A4' }
    : { bg: CT.greenSoft, fg: CT.greenDeep, border: `${CT.green}55` };
  return (
    <div style={{
      padding: '12px 14px', borderRadius: 14,
      border: `1px solid ${palette.border}`, background: palette.bg,
      color: palette.fg, fontSize: 13, lineHeight: 1.4, fontWeight: 550,
      ...style,
    }}>{children}</div>
  );
}
...
function ConfirmModal({ title, body, confirmLabel = 'Eliminar', onCancel, onConfirm, danger = true }) {
  return (
    <div style={{
      position: 'absolute', inset: 0, zIndex: 80, background: 'rgba(14,19,17,0.28)',
      display: 'flex', alignItems: 'flex-end', padding: 16,
    }}>
      <div style={{
        width: '100%', background: '#fff', borderRadius: 22, padding: 18,
        boxShadow: '0 24px 60px rgba(0,0,0,0.22)',
      }}>
        <div style={{ fontSize: 18, fontWeight: 700, color: CT.ink }}>{title}</div>
        <div style={{ fontSize: 14, color: CT.ink3, lineHeight: 1.45, marginTop: 6 }}>{body}</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 18 }}>
          <GhostButton onClick={onCancel} style={{ justifyContent: 'center', height: 48 }}>Cancelar</GhostButton>
          <button onClick={onConfirm} style={{
            height: 48, borderRadius: 12, border: 'none',
            background: danger ? CT.danger : CT.ink, color: '#fff',
...
Object.assign(window, {
  ...
  SectionLabel, Card, Label, TextInput, Notice, ScoreBar, ScreenHeader, ConfirmModal,
});
